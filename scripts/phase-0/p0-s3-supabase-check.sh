...
# Force a clean environment check by unsetting potential shell overrides
...
if [[ ! "$SUPABASE_KEY" =~ ^eyJ ]]; then
  crew_fail \
    ...
    --tool    "health_check" \
    --tool-args '{"fix": true}' \
    --context "The SUPABASE_KEY starts with '${SUPABASE_KEY:0:10}'. Project API keys (anon/service_role) must be JWTs starting with 'eyJ'. You may have accidentally used a Management API Key (sb_secret_...)." \
    --error   "Invalid key format: ${SUPABASE_KEY:0:10}... Expected JWT (eyJ...)"
  exit 1
fi
