...
# Detect timeout utility (macOS parity)
TIMEOUT_CMD="timeout"
if ! command -v timeout &>/dev/null && command -v gtimeout &>/dev/null; then
  TIMEOUT_CMD="gtimeout"
elif ! command -v timeout &>/dev/null; then
  echo "  ⚠ 'timeout' command not found. Falling back to un-timed curl."
  TIMEOUT_CMD=""
fi

# Use curl to open SSE for 3 seconds and grab the endpoint event
${TIMEOUT_CMD:+$TIMEOUT_CMD 3} curl -sN \
  -H "Accept: text/event-stream" \
  "${BRIDGE_URL}/sse" > "$SSE_LOG" 2>&1 || true

# Extract sessionId from endpoint event: data: /messages?sessionId=xxx
SESS_ID=$(grep -o 'sessionId=[^&"]*' "$SSE_LOG" | head -1 | cut -d= -f2 | tr -d '\r' || true)
