...
export const CREW_PERSONAS = {
  ...
  quark:           { role: 'Cost Optimization Specialist', goal: 'Maximize ROI and perform model arbitrage to keep missions under budget',    model: process.env.MODEL_COST_OPT     || 'google/gemini-flash-1.5' },
  tasha_yar:       { role: 'Tactical Readiness Officer',  goal: 'Execute system diagnostics and verify tactical readiness for deployment',   model: process.env.MODEL_ANALYST       || 'google/gemini-flash-1.5' },
};

export function normalisePersonaKey(name = '') {
  const key = name.toLowerCase()
    .replace(/^(captain|commander|lieutenant|lt\.|lt|counselor|dr\.|dr|chief|tactical)\s+/, (_, p) => {
      const m = { captain:'captain', commander:'commander', lieutenant:'lt', 'lt.':'lt', lt:'lt', counselor:'counselor', 'dr.':'dr', dr:'dr', chief:'chief', tactical:'tasha' };
      return (m[p.trim()] || p.trim()) + '_';
    })
    .replace(/[\s-]+/g, '_').replace(/[^a-z0-9_]/g, '');

  if (['tasha', 'lt_yar', 'yar', 'tactical_yar'].includes(key)) return 'tasha_yar';
  return key;
}
